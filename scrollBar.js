;(function ($) {
  $.fn.extend({
    "scrollBar": function (options) {
      ...
      var horizontalDiv = '<div class="zl-scrollBarBox" style="width:100%;left:0;height:' + settings.barWidth + 'px;bottom:0;background: '+settings.scrollbarColor+';">' +
        '<div class="zl-scrollBar zl-horizontalBar" style="height:' + settings.barWidth + 'px;border-radius:' + settings.barWidth / 2 + 'px;background: '+settings.sliderColor+';"></div>' +
        '</div>';
      var verticalDiv = '<div class="zl-scrollBarBox" style="height:100%;top:0;width:' + settings.barWidth + 'px;right:0;background: '+settings.scrollbarColor+';">' +
        '<div class="zl-scrollBar zl-verticalBar" style="width:' + settings.barWidth + 'px;border-radius:' + settings.barWidth / 2 + 'px;background: '+settings.sliderColor+';"></div>' +
        '</div>';
      var T = this;
      T.each(function () {
      ...
      var scrollHeigthOld =0; //滑块的原始高度
      var scrollHeightNew = 0;
      function creatScrollBar(obj) {
        var This = $(obj).get(0);
        var paddingR = parseFloat($(obj).attr("paddingR"));
        var paddingB = parseFloat($(obj).attr("paddingB"));
        $(obj).css({"padding-right": paddingR + "px", "padding-bottom": paddingB + "px", "overflow": "hidden"});
        ...
        function addVerticalBar(a) {
          ...
          scrollHeigthOld = $(a).find(".zl-scrollBarBox").find(".zl-verticalBar").attr("hideHeight");
          innerH = $(a).height();
          var barHeight = outerH * (innerH / scrollH);
          scrollHeightNew = barHeight;
          $(a).find(".zl-scrollBarBox").remove().end().append(verticalDiv).find(".zl-verticalBar").height(barHeight);
          $(a).find(".zl-scrollBarBox").remove().end().append(verticalDiv).find(".zl-verticalBar").attr("hideHeight",barHeight);
        };
          ...
          if (newWidth != oldWidth || newHeight != oldHeight || newInnerWidth != oldInnerWidth || newInnerHeight != oldInnerHeight) {
            ...
            if ($(obj).find(".zl-scrollBarBox").length) {
              if (topDiv) {
                ...
                scrollContentDiv.css({"left": leftDiv + 'px', "top": topDiv + 'px'});
              }
              ;
              if (topBar && $(obj).find(".zl-verticalBar").length) {
                var verticalBar = $(obj).find(".zl-verticalBar");
                var maxTop = parseFloat(topBar) + (parseFloat(scrollHeightNew) - parseFloat(scrollHeigthOld));
                if (topBar > maxTop) {
                  topBar = maxTop;
                }
              ...
              ;
            } else if (newHeight === oldHeight && newInnerHeight === oldInnerHeight) {
              scrollContentDiv.css({"top": '0px'});
            }
            oldWidth = newWidth;
            oldHeight = newHeight;
